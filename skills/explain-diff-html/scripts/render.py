#!/usr/bin/env python3
"""
render.py — render a structured explain-diff content spec into the self-contained
HTML page format used by the explain-diff-html skill (see ../SKILL.md).

Why this exists: the CSS, quiz JavaScript, and page scaffolding are identical across
every invocation of the skill — only the content (prose, diagrams, quiz questions)
actually changes per diff. Regenerating that boilerplate by hand each time wastes
tokens and drifts in quality. This script takes a small JSON spec with just the
content and renders the final page.

Credit: adapted from https://gist.github.com/ankitg12/8e808d387799de4e9839bc393f8e6405,
a fork of https://gist.github.com/geoffreylitt/a29df1b5f9865506e8952488eac3d524 that
factored the recipe's repeated CSS/JS scaffold out into this renderer.

Usage:
    python3 render.py spec.json [-o output.html]

If -o is omitted, writes to /tmp/YYYY-MM-DD-explanation-<slug>.html (matching the
...
manually vary position to "seem random", the script already handles it.

The "html" fields are raw HTML — write real markup (headings, <pre> blocks, tables,
".diagram"/".callout" divs per the class names below), not markdown. This keeps the
script a pure template renderer; all the writing judgment (what to explain, which
diagrams to draw) still belongs to the LLM following the skill, this just removes the
repetitive part.

Available diagram/content classes (for section "html" fields):
  .diagram          bordered container for a diagram
  .flow / .box      flex row of boxes connected by flow, e.g. <div class="flow">
                     <div class="box">A</div><span class="arrow">&rarr;</span>
                     <div class="box">B</div></div>
  .box.fail         a box styled to indicate an error/failure state
  .callout          highlighted aside for definitions, invariants, edge cases
  <pre><code>        code block; white-space: pre-wrap already applied
  <table>            plain HTML table for comparisons/mappings
"""
import argparse
import datetime
import html
...
document.querySelectorAll('.quiz-q').forEach(q => {
  q.querySelectorAll('.quiz-opt').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      fb.textContent = explanation ? verdict + ' ' + explanation : verdict;
...
def render(spec: dict) -> str:
    ...
    if quiz:
        ...
        for q in quiz:
            ...
            opts = []
            for o in options:
                explanation = html.escape(o.get("explanation", ""))
                opts.append(
                    f'<button class="quiz-opt" data-correct="{"true" if o["correct"] else "false"}" '
                    f'data-explanation="{explanation}">{o["text"]}</button>'
                )
            blocks.append(
